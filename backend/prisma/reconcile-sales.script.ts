...
function normalizePaymentMethodType(raw?: string | null): string {
  if (!raw) return 'CASH';
...
async function main() {
  ...
  const accountDeltas: Record<string, { name: string; amount: number }> = {};
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (order.payments && order.payments.length > 0) {
        for (const p of order.payments) {
          paymentSlices.push({
            method: normalizePaymentMethodType(p.paymentMethod?.type || order.paymentMethod),
            amount: p.amount,
