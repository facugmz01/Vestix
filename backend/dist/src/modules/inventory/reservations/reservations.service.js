...
let ReservationsService = class ReservationsService {
    ...
    async createReservation(payload) {
        const existing = this.holds.find(h => h.id === payload.cartId && h.status === reservation_model_1.ReservationStatus.ACTIVE);
        if (existing)
    ...
    async completeReservation(cartId) {
        ...
        reservation.status = reservation_model_1.ReservationStatus.COMPLETED;
    ...
    async cancelReservation(cartId) {
        ...
        reservation.status = reservation_model_1.ReservationStatus.CANCELLED;
        reservation.updatedAt = new Date();
        return reservation;
    }
    async sweepExpiredReservations() {
        const now = new Date();
        const expiredHolds = this.holds.filter(h => h.status === reservation_model_1.ReservationStatus.ACTIVE && h.expiresAt < now);
        let releasedCount = 0;
        for (const hold of expiredHolds) {
            ...
            hold.status = reservation_model_1.ReservationStatus.EXPIRED;
