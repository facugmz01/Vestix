...
async function bootstrap() {
    ...
    const [bizRows] = await db.query(`SELECT id, name FROM business LIMIT 1`);
    const [locRows] = await db.query(`SELECT id, name, city FROM business_locations ORDER BY id ASC LIMIT 1`);
    ...
    const [supplierRows] = await db.query(`SELECT id, supplier_business_name, name, email, mobile, tax_number, balance
     FROM contacts
    ...
    const [customerRows] = await db.query(`SELECT id, name, first_name, last_name, contact_type, email, mobile,
    ...
    const [allProducts] = await db.query(`SELECT p.id, p.name, p.type, p.sku, p.brand_id, p.category_id, p.product_description
     FROM products p
    ...
    const [stockRows] = await db.query(`SELECT vld.variation_id, SUM(vld.qty_available) as total_qty
    ...
    const [purchaseTxns] = await db.query(`SELECT t.id, t.contact_id, t.transaction_date, t.final_total, t.status, t.ref_no
     FROM transactions t
     WHERE t.type = 'purchase' AND t.is_suspend = 0
     ORDER BY t.transaction_date ASC`);
    const [allPurchasePayments] = await db.query(`SELECT tp.transaction_id, tp.amount FROM transaction_payments tp
    ...
    for (const tx of purchaseTxns) {
        ...
        if (!supplierId)
            continue;
        const [lines] = await db.query(`SELECT pl.variation_id, pl.quantity, pl.purchase_price_inc_tax, pl.purchase_price
       FROM purchase_lines pl WHERE pl.transaction_id = ?`, [tx.id]);
        if (lines.length === 0)
            continue;
        const validLines = lines.filter((l) => !!variantMap[l.variation_id]);
    ...
    const [sellTxns] = await db.query(`SELECT t.id, t.contact_id, t.transaction_date, t.final_total,
     ...
     ORDER BY t.transaction_date ASC`);
    const [allSellPayments] = await db.query(`SELECT tp.transaction_id, tp.amount, tp.method FROM transaction_payments tp
     INNER JOIN transactions t ON tp.transaction_id = t.id WHERE t.type = 'sell'`);
    const paysBySell = {};
    for (const sp of allSellPayments) {
        if (!paysBySell[sp.transaction_id])
            paysBySell[sp.transaction_id] = [];
    ...
    for (const tx of sellTxns) {
       ...
       WHERE tsl.transaction_id = ? AND tsl.parent_sell_line_id IS NULL`, [tx.id]);
        ...
        if (mainPay?.method) {
            const m = mainPay.method.toLowerCase();
            if (m.includes('bank_transfer') || m.includes('transfer'))
...
//# sourceMappingURL=migrate-ultimatepos-direct.js.map