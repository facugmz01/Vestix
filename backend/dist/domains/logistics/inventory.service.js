...
let InventoryService = class InventoryService {
    ...
    async findAllStock(query = {}) {
        ...
        const branchIds = [...new Set(data.map(d => d.branchId).filter(Boolean))];
    ...
    async findAllMovements(query = {}) {
        ...
        const warehouseIds = [...new Set(data.flatMap(m => [m.sourceWarehouseId, m.destinationWarehouseId]).filter(Boolean))];
        ...
        const enriched = data.map(m => {
            ...
            return {
                ...
                warehouseName: destinationWarehouse?.name || sourceWarehouse?.name || '',
            };
        });
        return { data: enriched, total, page, pageSize };
    }
